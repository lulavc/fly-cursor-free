<script setup>
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (!result.canceled) {
                // ElMessage.success(`Import completed! Successfully imported ${result.successful} accounts, failed ${result.failed} accounts.`);
                await fetchAccounts();
            } else {
    ...
    const importCurrentLoginAccount = () => {
        if (!cursorAccountsInfo.value.email) {
            ElMessage.error("No account is currently logged in.");
            return;
        }
    ...
    //         dialogVisible.value = true;
    //     } catch (error) {
                //         console.error("Edit operation error occurred:", error);
    ...
    const handleUpdateSessionToken = async (row) => {
        ...
        ElMessageBox.confirm("Using third-party interface to refresh 60-day token, this interface is limited to 5 times per IP per day, there may be unknown risks, are you sure to refresh?", "Notice", {
            confirmButtonText: "OK",
            cancelButtonText: "Cancel",
            type: "warning",
        }).then(async () => {
    ...
    const handleSizeChange = (val) => {
        ...
        pagination.currentPage = 1; // Return to first page when changing page size
        fetchAccounts();
    };
